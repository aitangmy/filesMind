...
class FixMarkdownHierarchyTests(unittest.TestCase):
    def test_snap_prefers_shallower_level_when_jitter_is_ambiguous(self):
        ...
        snapped = ps._snap_orig_level_for_stack(4, level_stack, jitter_tolerance=1)
    ...
    def test_numbering_resolution_prefers_sibling_level_over_parent_level(self):
        ...
        level = ps._resolve_level_by_numbering((1, 3), history, fallback_level=3)
        self.assertEqual(level, 5)

    def test_fix_markdown_hierarchy_repairs_numbered_parent_child_chain(self):
        source = "\n".join(
            [
                "## 1. 概述",
                "## 1.1 背景",
        ...
        fixed = ps.fix_markdown_hierarchy(source).splitlines()
        self.assertEqual(fixed[0], "## 1. 概述")
        self.assertEqual(fixed[1], "### 1.1 背景")
        self.assertEqual(fixed[2], "#### 1.1.1 定义")

    def test_fix_markdown_hierarchy_repairs_numbered_siblings_from_history(self):
        source = "\n".join(
            [
                "## 1.1 背景",
                "###### 1.2 发展阶段",
    ...
    def test_fix_markdown_hierarchy_applies_non_numbered_anti_jump(self):
        source = "\n".join(
            [
                "## 1.1 背景",
                "###### 结论",
    ...
    def test_fix_markdown_hierarchy_prevents_staircase_on_repeated_noisy_headings(self):
        source = "\n".join(
            [
                "## 1. 核心理论",
                "###### 假标题A",
    ...
    def test_fix_markdown_hierarchy_handles_ocr_jitter_for_non_numbered_siblings(self):
        source = "\n".join(
            [
                "## 1. 核心理论",
                "#### 假标题A",
    ...
    def test_fix_markdown_hierarchy_keeps_shallow_context_after_deep_numbered_anchor(self):
        source = "\n".join(
            [
                "## 1. 架构",
                "#### 无编号分组",
